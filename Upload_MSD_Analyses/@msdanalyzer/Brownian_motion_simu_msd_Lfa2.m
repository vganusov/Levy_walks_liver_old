...
num=xlsread('All_datasets.xlsx','Trial2');%'AllData_Wk1'); 
%num=xlsread('Trial_lfaKO_2.xlsx','Sheet1'); 
Table=array2table(num, 'VariableNames',{'Dnumber'	'Ddiscription'...
...
%num=xlsread('All_datasets.xlsx','Trial'); 
%Table=array2table(num, 'VariableNames',{'Movie'	'x'	'y' 'z'...
%    'Unit'	'Category'	'Collection'	'Time_Slice'	'TrackID'...
%    'ID'	'Time'	'Dis'	'Time_diff'	'Speed'	'Cell_ID'});
...
UA=unique(Table.UniqueTcellID);
%UA=unique(Table.Cell_ID);
[co xx]= size(UA);
N_PARTICLES=co; 
%N_TIME_STEPS = 100;
N_DIM = 3; % 2D
...
polyfit(log(x+0.0000001),log(t+0.000001),1)