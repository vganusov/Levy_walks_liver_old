...
if errorbar
    h = msdanalyzer.errorShade(ha, log10(msmsd(:,1)), log10(msmsd(:,2)), log10(msmsd(:,3)), [0 0 0], false);
    set(h.mainLine, 'LineWidth', 2);
    
else
    h = plot(ha, log10(msmsd(:,1)), log10(msmsd(:,2)),'>b');
    xlim([1 4])
    ylim([0 5])
    %h = plot(ha, log10(msmsd(:,1)), log10(msmsd(:,2)), 'b', '>', 'LineWidth', 2);
end
