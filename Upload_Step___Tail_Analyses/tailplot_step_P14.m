function h=tailplot_step_P14(x, xmin, alpha)
% 
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        h = loglog(c(:,1),c(:,2),'b*','MarkerSize',8,'MarkerFaceColor',[1 1 1]); hold on;
        loglog(cf(:,1),cf(:,2),'b--','LineWidth',2); hold off;
        ...
        ylabel('Pr(r \geq b)');%,'FontSize',9.35);
        xlabel('Step lengths, r (\mum)');%,'FontSize',9.35)
    ...
    case 'INTS', 
        ...
        h(2) = loglog(cf(:,1),cf(:,2),'b--','LineWidth',2); hold off;
        xr  = [10.^floor(log10(min(x))) 10.^ceil(log10(max(x)))];
        xrt = (round(log10(xr(1))):2:round(log10(xr(2))));
        if length(xrt)<4, xrt = (round(log10(xr(1))):1:round(log10(xr(2)))); end;
        yr  = [10.^floor(log10(1/n)) 1];
