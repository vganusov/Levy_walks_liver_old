% Computes branch length distribution -LN
...
branch_l=branch_l*(300/512);
bran_len_str=bran_len_str*(300/512);
...
dt2=1;
subplot(1,4,1)
xlabel('Branch length (B, \mum)');ylabel('Prb(B)')
subplot(1,4,4)
dt4=0.05;
histogram(bran_len_str_f./branch_lf, 'Normalization','probability','BinWidth',dt4);
xlabel('Branch straightness index (linear dist/path)');ylabel('Prb(straightness)')
cou=1;
...
subplot(1,4,3)
dt5=10;
histogram(real(br_ang2_f),'Normalization','probability','BinWidth',dt5); 
...
legend('Data','Caushy 1p','Levy 1p',...
   ...
   'Half Normal 1p')
xlabel('Branch lengths(\mum)');ylabel('Prb')
